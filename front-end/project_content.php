		...
		<div id="main-content">
			<div class="w3-row">
				<div class="w3-col l9 m9 s12" style="border-right: solid 1px #ccc; height: 100%; padding-top: 20px;">
					<?php
					if(isset($_GET['id'])) {
						$id_hangmucthicong = $_GET['id'];
						settype($id_hangmucthicong,"int");
					}
					?>
					<?php
					$qr_hangmucthicong = "SELECT * FROM hangmucthicong WHERE id=$id_hangmucthicong";
					$hangmucthicong = mysqli_query($conn,$qr_hangmucthicong);
					$row_hangmucthicong = mysqli_fetch_assoc($hangmucthicong);
					?>
					
					<?php
					$id_loaihangmuc = $row_hangmucthicong['idLoaiHangMuc'];
					$qr_loaihangmuc = "SELECT * FROM loaihangmucthicong WHERE id=$id_loaihangmuc";
					$loaihangmuc = mysqli_query($conn,$qr_loaihangmuc);
					$row_loaihangmuc = mysqli_fetch_assoc($loaihangmuc);
					?>
					<div style="text-align: center;"><a href="#" style="color: #555555;font-size: 0.8em;"><?php echo $row_loaihangmuc['name'] ?></a></div>
