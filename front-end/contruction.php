...
$qr_congtrinh = "SELECT * FROM congtrinh";
$congtrinh = mysqli_query($conn,$qr_congtrinh);
$sotin = 10;
$tong = mysqli_num_rows($congtrinh);
if($tong > $sotin) {
			...
			<div class="w3-row">
				<div class="w3-col l9" style="border-right: solid 1px #ccc; height: 100%;">
					<div id="title-content" class="w3-center">
						<h4>CATEGORY ARCHIVES: CÔNG TRÌNH</h4>
					</div>
					<div id="news-contruction">
						<?php
								$qr_congtrinh = "SELECT * FROM congtrinh LIMIT $start,$sotin";
								$congtrinh = mysqli_query($conn,$qr_congtrinh);
								while($row_congtrinh = mysqli_fetch_assoc($congtrinh)) {
						?>
						<div class="w3-row" style="margin-bottom: 16px;">
							<a href="contruction_content.php?id=<?php echo $row_congtrinh['id'] ?>">
								<div class="w3-col l5 s12">
									<div class="box-img">
										<img src="image/hangmucthicong/<?php echo $row_congtrinh['image'] ?>" alt="" width="100%" height="250px;">
									</div>
								</div>
								<div class="w3-col l7 s12">
									<div class="box-news">
										<div class="box-news-tittle">
											<?php echo $row_congtrinh['tieuDe'] ?>
										</div><hr>
										<div class="box-news-summary">
											<?php echo $row_congtrinh['tomTat'] ?>
										</div>
									</div>
								</div>
							</a>
					...
					<div class="w3-center">
						<div class="w3-bar">
							...
							if($page > 1) {
								$prev = $page - 1;
							?>
  							<a href="contruction.php?page=<?php echo $prev ?>" class="w3-bar-item w3-button w3-circle w3-border">&laquo;</a>
							<?php  } ?>
							<?php
							// Nut so
							for($i = 1;$i <= $sotrang;$i++) {
								if($page != $i) {
							?>
  							<a href="contruction.php?page=<?php echo $i ?>" class="w3-bar-item w3-button w3-circle w3-border"><?php echo $i ?></a>
							<?php } else { ?>
							...
							if($page < $sotrang) {
								$next = $page + 1;
							?>
  							<a href="contruction.php?page=<?php echo $next ?>" class="w3-bar-item w3-button w3-circle w3-border">&raquo;</a>
							<?php } ?>
						</div>
					</div>
					<?php } ?>
